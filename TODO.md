...
<!-- 자유 메모 — /interview 1에서 읽어 질문 건너뜀:
- `npx create-next-app@latest` (TS, App Router, Tailwind) + 기존 파일과 공존
- 의존성: @anthropic-ai/sdk, @supabase/ssr, katex, zod, vitest
- vercel.ts (vercel.json 아님), .env.local.example
- Supabase 프로젝트 생성 (Vercel Marketplace 또는 MCP) + profiles/students 마이그레이션 + RLS
- Auth: magic link, role=teacher|student, middleware.ts에서 라우트 가드
- 최소 화면: /login, /(teacher)/dashboard, /(student)/home (빈 화면 + role guard만)
- 성공 기준: teacher/student 계정으로 각자 라우트에 접근 / 반대 라우트 차단 확인
- 참고 plan: ~/.claude/plans/velvet-wishing-naur.md
-->
